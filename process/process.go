...
func (p *Process) GetDescription() string {
	...
	if p.state == RUNNING {
		...
		return p.stopTime.Format(time.RFC3339)
	}
	return ""
}
...
func (p *Process) GetPid() int {
	...
	if p.cmd == nil ||
		p.cmd.Process == nil ||
		p.state == STOPPED || p.state == FATAL || p.state == UNKNOWN || p.state == EXITED || p.state == BACKOFF {
		return 0
	}
...
func (p *Process) createProgramCommand() (err error) {
	// it will panic when configuration is error, but supd need to work stable because it is a daemo process.
	defer func() {
		if e := recover(); e != nil {
			debug.PrintStack()
			// make an error to return
			err = errors.New("panic").As(e)
		}
	}()
...
func (p *Process) run(finishCb func(code int)) {
	p.lock.Lock()
	defer p.lock.Unlock()
	...
	for !p.stopByUser {
		...
		if atomic.LoadInt32(p.retryTimes) >= p.getStartRetries() {
			// log.WithFields(log.Fields{"program": p.GetName()}).Error(fmt.Sprintf("fail to start program because retry times is greater than %d", p.getStartRetries()))
			// finishCb(-1)
			p.failToStartProgram(fmt.Sprintf("fail to start program because retry times is greater than %d", p.getStartRetries()), finishCbWrapper, 0)
			break
		}
	}
...
func (p *Process) setLog() error {
	if p.config.IsProgram() {
		var err error
		p.StdoutLog, err = p.createLogger(
			p.GetStdoutLogfile(),
		...
		if p.config.GetBool("redirect_stderr", false) {
			...
			p.StderrLog, err = p.createLogger(
				p.GetStderrLogfile(),
...
func (p *Process) createLogger(logFile string, maxBytes int64, backups int, logEventEmitter logger.LogEventEmitter) (logger.Logger, error) {
